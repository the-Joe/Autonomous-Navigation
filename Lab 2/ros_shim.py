...
class SubscriberWrap():
    ...
    def __init__(self, topic, msg_type, callback, parentNode, sr=0.1):
        self.topic = topic
        self.rate = sr
        self.msg_type = msg_type()
        self.parent = parentNode
...
class Rospy():
    ...
    def Subscriber(self, topic, msgType, callback):
        if topic in list(self.subscriptions.keys()):
    ...
    def init_node(self, node_name):
        print("Starting Node for %s"% node_name)
    ...
    def __del__(self):
        for tp, ob in self.subscriptions.items():
            del ob
